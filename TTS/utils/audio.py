...
class AudioProcessor(object):
    ...
    def compute_f0(self, x: np.ndarray) -> np.ndarray:
        ...
        # f0, _, _, _ = compute_yin(x, self.sample_rate, self.win_length, self.hop_length, 65 if self.mel_fmin == 0 else self.mel_fmin, self.mel_fmax)
        ...
        f0, _, _ = librosa.pyin(x, fmin=65 if self.mel_fmin == 0 else self.mel_fmin, fmax=self.mel_fmax, frame_length=self.win_length, sr=self.sample_rate, fill_na=0.0)
